...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("#calText").text("確定取消變更文件?(按確定後無法復原)");
		$("#calDialog").dialog({
			autoOpen: false,
			modal:true
		});
...
<div id="calDialog"><span id="calText"></span><hr/><div id="calBtn"><input type="button" class="button60" value="確定" onClick='calHover()' />&nbsp;&nbsp;&nbsp;<a href="#" onClick='calAdd()' />我反悔了</a></div><input type="hidden" id="calUrl" value=""></div>
	<div id="content">
        ...
        <h4><{$func_Cname}></h4>
      <p class="notes">(*為必填)</p>
	  <form name="addForm" id="addForm" method="POST" action="<{$do_edit_doc_url}>" enctype="multipart/form-data">
        <table summary="services">
            ...
            <tbody>
              <tr>
				...
				<td>
					<input type="field" name="PublishDate" value="<{$PublishDate}>" id="datepicker" class="required date" /> 
				</td>
              </tr>	
              <tr class="odd">
				<th scope="row" class="column1">* 文件標題</th>
				<td>
				<input type="text" name="caption" value="<{$caption}>" maxlength="100" class="required" /> 
				</td>
              </tr>	
              <tr>
				<th scope="row" class="column1">* 上傳文件</th>
				<td>
					<a href="<{$uploadPath}>docs_<{$doc_id}><{$doc_type}>"><{$name}></a>
              ...
              <tr class="odd">
				...
				<td>
				<textarea name="desc" cols="" rows=""><{$doc_desc}></textarea>
				</td>
              </tr>	
              <tr>
