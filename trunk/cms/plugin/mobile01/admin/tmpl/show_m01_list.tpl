...
<div id="delDialog"><span id="delText"></span><hr/><div id="delBtn"><input type="button" class="button60" value="確定" onClick='delHover()' />&nbsp;&nbsp;&nbsp;<a href="#" onClick='calDel()' />我反悔了</a></div><input type="hidden" id="delUrl" value=""></div>
  <div id="content">
        ...
        <h4><{$func_Cname}><input type="button"name="button"class="button60" value="新增文章" onClick="aHover('<{$show_add_m01_link}>')" title=""/></h4>
		
        <div id="sub_navi">
	        <label id="current_keyword">目前關鍵字</label>	
			<form action="<{$show_m01_list_link}>" method="POST" >
...
<{/foreach}>
		...
		<hr/>
		<table width="500px" >
	<{foreach from="$m01_data" item="rows" name="m01_data"}>
		  <tr height="100px">
			<td>
			<input type="button"name="button"class="button62grey" value="修改" onClick="aHover('<{$show_edit_m01_link}><{$rows.m01_id}>')" title=""/>
			<input type="button"name="button"class="button60grey" value="刪除" onClick="showDelDialog('<{$do_del_m01_link}><{$rows.m01_id}>')" title=""/>
			</td>
			<td>
			文章標題：<U><{$rows.m01_title}></U><br />
			發表日期： <{$rows.m01_post_time}>
			</td>
		  </tr>
	<{foreachelse}>
	<tr>
