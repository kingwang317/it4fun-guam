...
<div id="delDialog"><span id="delText"></span><hr/><div id="delBtn"><input type="button" class="button60" value="確定" onClick='delHover()' />&nbsp;&nbsp;&nbsp;<a href="#" onClick='calDel()' />我反悔了</a></div><input type="hidden" id="delUrl" value=""></div>
  <div id="content">
        ...
        <h4><{$func_Cname}><input type="button"name="button"class="button60" value="新增文章" onClick="aHover('<{$show_add_ptt_link}>')" title=""/></h4>
		
        <div id="sub_navi">
	        <label id="current_keyword">目前關鍵字</label>	
			<form action="<{$show_ptt_list_link}>" method="POST" >
...
<{/foreach}>
		...
		<hr/>
		<table width="500px" >
	<{foreach from="$ptt_data" item="rows" name="ptt_data"}>
		  <tr height="100px">
			<td>
			<input type="button"name="button"class="button62grey" value="修改" onClick="aHover('<{$show_edit_ptt_link}><{$rows.ptt_id}>')" title=""/>
			<input type="button"name="button"class="button60grey" value="刪除" onClick="showDelDialog('<{$do_del_ptt_link}><{$rows.ptt_id}>')" title=""/>
			</td>
			<td>
			文章標題：<U><{$rows.ptt_title}></U><br />
			發表日期： <{$rows.ptt_post_time}><br />
			追蹤日期： <{$rows.ptt_track_time}><br />
			看板位置： <{$rows.ptt_bid}><br />
			文章代碼： <{$rows.ptt_pid}>
			</td>
		  </tr>
	<{foreachelse}>
