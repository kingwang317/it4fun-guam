<?php 
		...
		function set_value($id,$value,$type){
			$db = init_db();
			$sql = "UPDATE plu_blog SET blog_$type = '$value' WHERE blog_id = '$id'";
			$db->query($sql);
		}
		if(isset($_GET['type'])){
			switch($_GET['type']){
				case "blog_status":
				set_value($_POST['blog_status_id'],$_POST['blog_status_dialog'],"status");
				break;
				case "blog_cate":
				set_value($_POST['blog_cate_id'],$_POST['blog_cate_dialog'],"cate");
				break;
				case "blog_rank":
		...
		if(isset($_GET['order_type'])){
			...
			case "posti":
			$order_by = " ORDER BY blog_post_time DESC ";
			break;
			case "postd":
			$order_by = " ORDER BY blog_post_time ASC ";
			break;			
			case "statusi":
			$order_by = " ORDER BY blog_status DESC ";
			break;
			case "statusd":
			$order_by = " ORDER BY blog_status ASC ";
			break;
			default:
			break;
			}
		...
		$dataTotal = $db->get_var("SELECT COUNT(blog_id) FROM plu_blog" . $filter);
		list($dataStart, $dataLen, $pagejump) = init_sliding_pager($dataTotal);
		$sql = "SELECT * FROM plu_blog ". $filter . $order_by ." LIMIT $dataStart, $dataLen";
		$result = $db->get_results($sql);
		$export_sql = "SELECT blog_post_time,blog_cate,'所有部落格',blog_rank,blog_title,blog_url,'' as qus1,'' as qus2,kw_ids FROM plu_blog ". $filter . $order_by ."LIMIT 500";
		//$export_sql = "SELECT * FROM plu_blog ". $filter . $order_by ."LIMIT 500";
...
<script type="text/javascript">
	...
	function postVlaue(type){
		$('#'+type+'Dialog').dialog("close");
		$.post("<?php echo WEBROOT ?>/index.php?cmsid=3&type=blog_"+type+"", $("#"+type+"_form").serialize());
		location.reload();
	}
	function showDialog(type,id,value) {
		$("#"+type+"Dialog").dialog("open");
		$("#blog_"+type+"_id").val(id);
		$('input[name=blog_'+type+'_dialog]:radio').filter('[value='+value+']').attr('checked', true);
	}
...
<h4>部落格口碑文章</h4>
        <div id="select">
		<form name="selForm" id="selForm" action="#remote-tab-1" method="GET" >
		<table class="praise">
		  <tr>
			<td class="k1">選擇關鍵字:</td>
			<td class="k2">
			<select name="blog_kw_sel" >
	    ...
	    <input name="range" id="range" type="radio" value="1" />最新回應日期</td>
			...
			<td><input name="end_date" id="end_date" type="text" /></td>
			 <input name="blog_is_search_bar" type="hidden" value="true" />
...
//$("select").val(["<?php echo $_GET['blog_kw_sel']; ?>","<?php echo $_GET['blog_status']; ?>","<?php echo $_GET['blog_cate']; ?>","<?php echo $_GET['blog_rank']; ?>"]);
</script>
<?php 
}
?>	  
          ...
          <hr/>
			  ...
			  <br clear="all"/>
					 ...
					 <a href="<?php echo WEBROOT."/?".str_replace('cmsid=3','',$_SERVER['QUERY_STRING']) ?>&order_type=posti#remote-tab-1" <?php if($order_type=="posti")echo 'class="order-style"'?>>發表日期較新</a> |
					 <a href="<?php echo WEBROOT."/?".str_replace('cmsid=3','',$_SERVER['QUERY_STRING']) ?>&order_type=postd#remote-tab-1" <?php if($order_type=="postd")echo 'class="order-style"'?>>發表日期較舊</a> |
					 <a href="<?php echo WEBROOT."/?".str_replace('cmsid=3','',$_SERVER['QUERY_STRING']) ?>&order_type=statusi#remote-tab-1" <?php if($order_type=="statusi")echo 'class="order-style"'?>>狀態已處理 </a>|
					 <a href="<?php echo WEBROOT."/?".str_replace('cmsid=3','',$_SERVER['QUERY_STRING']) ?>&order_type=statusd#remote-tab-1" <?php if($order_type=="statusd")echo 'class="order-style"'?>>狀態未處理 </a>
					</div>
					<br/>
                     <div id="top-next"><?php print_r(str_replace("cmsid=3","",$pagejump)); ?></div>
				<table class="tab" name="blog_table" id="blog_table">
					<thead>
					   <tr>
...
?>
		<tr >
			...
			<td class="column1"><u onClick="showDialog('status','<?php echo $rows->blog_id ?>','<?php echo $rows->blog_status ?>');" ><?php echo $rows->blog_status?></u></td>
			<td><u><a href="<?php echo $rows->blog_url ?>"  target="_blank"  ><?php echo $rows->blog_title?></a></u></td>
			<td class="column1"><u onClick="showDialog('cate','<?php echo $rows->blog_id ?>','<?php echo $rows->blog_cate ?>');" ><?php echo $rows->blog_cate?></u></td>
			<td class="column1"><u onClick="showDialog('rank','<?php echo $rows->blog_id ?>','<?php echo $rows->blog_rank ?>');" ><?php echo $rows->blog_rank?></u></td>
			<td class="column1"><?php echo $rows->blog_post_time?></td>
		</tr>
<?php
				}
			}
?>
        ...
        <div id="bottom-next"><?php print_r(str_replace("cmsid=3","",$pagejump)); ?></div>

<div id="statusDialog" title="變更狀態" style="display:none" >
<form name="status_form" id="status_form" method="POST" >
<input type="hidden" name="blog_status_id" id="blog_status_id" />
<input name="blog_status_dialog" checked="true" type="radio" value="未處理" />未處理<br/>
<input name="blog_status_dialog" type="radio" value="處理中" />處理中<br/>
<input name="blog_status_dialog"  type="radio" value="已處理" />已處理<br/>
<input name="blog_status_dialog" type="radio" value="不處理" />不處理<br/>
...
<input type="hidden" name="blog_cate_id" id="blog_cate_id" />
<input name="blog_cate_dialog" checked="true" type="radio" value="品牌" />品牌<br/>
<input name="blog_cate_dialog" type="radio" value="產品" />產品<br/>
<input name="blog_cate_dialog"  type="radio" value="售後服務" />售後服務<br/>
<input name="blog_cate_dialog" type="radio" value="銷售" />銷售<br/>
<input name="blog_cate_dialog" type="radio" value="行銷活動" />行銷活動<br/>
<input name="blog_cate_dialog"  type="radio" value="主動宣傳" />主動宣傳<br/>
<input name="blog_cate_dialog" type="radio" value="其他" />其他<br/>
...
<input type="hidden" name="blog_rank_id" id="blog_rank_id" />
<input name="blog_rank_dialog" checked="true" type="radio" value="未分類" />未分類<br/>
<input name="blog_rank_dialog" type="radio" value="正面" />正面<br/>
<input name="blog_rank_dialog"  type="radio" value="一般" />一般<br/>
<input name="blog_rank_dialog" type="radio" value="負面" />負面<br/>
<input name="blog_rank_dialog" type="radio" value="惡意攻擊" />惡意攻擊<br/>
